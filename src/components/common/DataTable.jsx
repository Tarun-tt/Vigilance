  ...
  return (
    ...
    >
      <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.35 }}>
        {children}
        {sortable && <UnfoldMoreIcon sx={{ fontSize: 18, color: boiBlue, opacity: 0.55 }} />}
      </Box>
    </TableCell>
  );
}
...
export function DataTable({
  ...
  const sliceEnd = controlledPage === undefined ? sliceStart + rowsPerPage : rows.length;
  const visibleRows = controlledPage === undefined ? rows.slice(sliceStart, sliceEnd) : rows;

  const sortableCol = (id) => id === 'sn' || id === 'status';
      ...
      <TableContainer>
        <Table size="small" stickyHeader>
          <TableHead>
            <TableRow>
              {columns.map((col) => (
                <HeaderCell key={String(col.id)} sortable={sortableCol(col.id)} align={col.align}>
                  {col.label}
                </HeaderCell>
              ))}
            </TableRow>
